...
const CLUSTER_LABEL: Record<Cluster, string> = { A: "🔵 group A", B: "🟣 group B" };

class SemanticConstellation implements GameInstance {
  ...
  async init(packPath: string): Promise<void> {
    ...
    const idx = resolveDailyPuzzleIndex(
      "semantic-constellation", dayId, this.pack.contentPackVersion, this.pack.datasetId, this.pack.puzzleCount,
      { read: (k) => this.svc.storage.read(k) },
      (word) => this.pack.puzzles.findIndex((p) => p.anchorA === word.toLowerCase() || p.anchorB === word.toLowerCase() || p.answers.some((a) => a.word === word.toLowerCase())),
    );
  ...
  private commitTrace(): void {
    ...
    if (out.found && out.error === null) {
      ...
      this.persist();
      if (out.found.type === "theme" && out.found.cluster) this.announce(`Theme word — ${CLUSTER_LABEL[out.found.cluster]}.`);
  ...
  render(): void {
    ...
    this.root.append(homeBar(this.svc, "Semantic Constellation"));
    ...
    this.root.append(el("p", { class: "help", text: "Trace words along touching tiles. Four words split into two hidden groups (🔵 A / 🟣 B); one 8-letter word bridges both and spans the grid. Bonus words earn hints." }));
    ...
    if (foundThemes.length > 0) {
      ...
      for (const a of foundThemes) legend.append(el("span", { class: `sg-band sc-${a.cluster}`, text: `${a.word} · ${CLUSTER_LABEL[a.cluster!]}` }));
      this.root.append(legend);
    }

    if (s.status === "won") {
      this.root.append(el("p", { class: "win", text: `Solved! Group A ≈ “${this.puzzle.anchorA}”, Group B ≈ “${this.puzzle.anchorB}”.` }));
    }
  }
...
export const semanticConstellationPlugin: GamePlugin = {
  meta: { id: "semantic-constellation", name: "Semantic Constellation", tagline: "Split the words into two hidden stars, bridged by a spangram.", glyph: "✨", accent: "#8a3ffc" },
  contentPackPath: "./semantic-constellation.json",
  async mount(root, services) {
    const game = new SemanticConstellation(root, services);
    await game.init("./semantic-constellation.json");
    game.render();
    return game;
  },
};
