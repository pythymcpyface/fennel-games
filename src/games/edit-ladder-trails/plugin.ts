...
class EditLadderTrails implements GameInstance {
  ...
  async init(packPath: string): Promise<void> {
    ...
    const idx = resolveDailyPuzzleIndex(
      "edit-ladder-trails", dayId, this.pack.contentPackVersion, this.pack.datasetId, this.pack.puzzleCount,
      { read: (k) => this.svc.storage.read(k) },
      (word) => this.pack.puzzles.findIndex((p) => p.spangram === word.toLowerCase() || p.ladder.includes(word.toLowerCase())),
    );
  ...
  private commitTrace(): void {
    ...
    if (out.found && out.error === null) {
      ...
      this.persist();
      const label = out.found.type === "spangram" ? "the spangram" : out.found.type === "rung" ? "a ladder word" : "a bonus word (hint earned)";
      this.announce(`Found ${label}!`);
  ...
  private doHint(): void {
    ...
    this.persist();
    if (out.revealed) {
      const kind = out.revealed.type === "spangram" ? "spangram" : out.revealed.type === "rung" ? "ladder word" : "bonus word";
  ...
  render(): void {
    const s = this.state;
    this.root.innerHTML = "";
    this.root.className = "game edit-ladder-trails";
    this.root.append(homeBar(this.svc, "Edit-Ladder Trails"));
    ...
    this.root.append(el("p", { class: "help", text: "Trace words along touching tiles (any direction). Four 5-letter words form an edit ladder; one 8-letter word spans the grid. Bonus words earn hints." }));
    ...
    // Ladder progress (spoiler-free: shows found rungs as their word, unfound as dashes).
    const ladderLine = el("p", { class: "elt-ladder" });
    ladderLine.append(el("span", { class: "elt-ladder-label", text: "Ladder: " }));
    this.puzzle.ladder.forEach((w, i) => {
      const found = this.puzzle.answers.some((a) => a.type === "rung" && a.word === w && s.foundIds.includes(a.id));
      if (i > 0) ladderLine.append(el("span", { class: "elt-arrow", text: " → " }));
      ladderLine.append(el("span", { class: found ? "elt-rung found" : "elt-rung", text: found || s.status === "won" ? w : "•••••" }));
    });
    this.root.append(ladderLine);

    if (s.status === "won") {
      this.root.append(el("p", { class: "win", text: `Solved! The spangram was “${this.puzzle.spangram}”.` }));
    }
  }
...
export const editLadderTrailsPlugin: GamePlugin = {
  meta: { id: "edit-ladder-trails", name: "Edit-Ladder Trails", tagline: "Trace a ladder of one-letter changes across the grid.", glyph: "🪜", accent: "#4589ff" },
  contentPackPath: "./edit-ladder-trails.json",
  async mount(root, services) {
    const game = new EditLadderTrails(root, services);
    await game.init("./edit-ladder-trails.json");
    game.render();
    return game;
  },
};
